...
nll <- function(par){
  ps <- matrix(0,nrow=m,ncol=J)
  ps[,J] <- 1
  for(i in 1:m){
    for(j in 1:(J-1)){
      ps[i,j] <- exp(sum(x[i,]*par[((j-1)*K+1):(j*K)]))
    }
  }
  ps <- ps/rowSums(ps)
  return(-sum(taxa*log(ps)))
}

eps <- 100
nll_ent <- function(par){
  ps <- matrix(0,nrow=m,ncol=J)
  ps[,J] <- 1
  for(i in 1:m){
    for(j in 1:(J-1)){
  ...
  ps <- ps/rowSums(ps)
  return(-sum((taxa-eps*ps)*log(ps)))
}


# par <-as.vector(t(0.99*(taxa[,-J]/rowSums(taxa))+0.005/J))
par <- rep(0,(J-1)*K)
fit <- optim(par=par,fn=nll, control=list(trace=2,maxit=100),
             method = "BFGS")
fit <- optim(par=par,fn=nll, control=list(trace=2,maxit=1000))
fit$par

fit_ent <- optim(par=par,fn=nll_ent, control=list(trace=2,maxit=100),
             method = "BFGS")
fit_ent <- optim(par=par,fn=nll_ent, control=list(trace=2,maxit=500))
fit_ent$par

sum((fit$par-par)^2)
sum((fit_ent$par-par)^2)
betas <- matrix(fit$par, nrow=J-1,ncol=K,byrow=TRUE)
